...
public:
	...
	virtual void ApplySchemeSettings(vgui2::IScheme *pScheme) override
	{
		...
		if (m_pFullText)
		{
			if (m_pKeyboard)
			{
				const wchar_t hotkey = m_cHotkey;
				SetText(m_pText);
				SetHotkey(hotkey);
			}
		}
	...
	{
		vgui2::localize()->ConvertUnicodeToANSI(m_pFullText ? m_pFullText : L"", textOut, bufferLen);
	}

	void GetText(wchar_t *textOut, int bufLenInBytes)
	{
		const size_t capacity = bufLenInBytes > 0 ? bufLenInBytes / sizeof(wchar_t) : 0;
		if (capacity)
		{
			wcsncpy(textOut, m_pFullText ? m_pFullText : L"", capacity - 1);
			textOut[capacity - 1] = L'\0';
		}
	}
	...
	{
		// Callers may pass m_pText itself while a scheme is being reapplied.
		const std::wstring textCopy(wtext ? wtext : L"");
		wtext = textCopy.c_str();
		...
		m_pFullText = new wchar_t[textCopy.size() + 1];
		std::copy(textCopy.begin(), textCopy.end(), m_pFullText);
		m_pFullText[textCopy.size()] = L'\0';
		m_pText = m_pFullText + (wtext - saveptr);
		m_cHotkey = hotkey;

		BaseClass::SetText(m_pKeyboard ? m_pText : m_pFullText);
		SetHotkey(hotkey);
	}
	...
	void SetText(const char *text)
	{
		...
		const wchar_t* localized = text && text[0] == '#' ? vgui2::localize()->Find(text) : nullptr;
		if (!localized)
		{
			vgui2::localize()->ConvertANSIToUnicode(text ? text : "", unicodeText, sizeof(unicodeText));
			localized = unicodeText;
		}
		SetText(localized);
	}
