...
def smoke(session, run_root, data_root, timeout):
    ...
    session.settle(1)
    session.send("clientinfo 0;bot_quota 4")
    ...
    session.send("moe_buy weapon_m134;primammo")
    session.settle(1.2)
    session.send("slot1")
    session.settle(1.2)
    session.send("+attack")
    ...
    if not re.search(r"(?m)^\s*weapon_m134\s*$", session.text[start:]):
        raise RuntimeError("weapon_m134 was not found in the server entity dump")
