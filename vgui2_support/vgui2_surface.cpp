...
extern "C" rgba_t g_color_table[8];
extern BaseUISurface *staticSurface;

extern "C" void VGUI2_Draw_Init()
{
    if (!vgui2::scheme())
        return;
...
{
    if (!vgui2::surface() || font == NULL_HANDLE)
        return ch ? 8 : 0;
    int a, b, c;
...
extern "C" int VGUI2_Surface_GetCharWidth(int ch)
{
    return VGUI2_GetFontWide(ch, _consoleFont);
}

extern "C" int VGUI2_Surface_GetCharHeight()
{
...
extern "C" int VGUI2_Surface_DrawChar(int x, int y, int ch, byte r, byte g, byte b, byte a)
{
    if (!vgui2::surface() || _consoleFont == NULL_HANDLE)
        return VGUI2_GetFontWide(ch, _consoleFont);
    BaseUISurface::ScreenSpaceGuard screenSpace(*staticSurface);
...
extern "C" void VGUI2_Surface_DrawStringLen( const char* pText, int* length, int* height )
{
    if(length)
...
extern "C" int VGUI2_Surface_DrawConsoleString(int x0, int y0, const char* string, byte r, byte g, byte b, byte a)
{
    if (!vgui2::surface() || _consoleFont == NULL_HANDLE)
        return x0;
    BaseUISurface::ScreenSpaceGuard screenSpace(*staticSurface);
    ...
    const std::array<byte, 4> col = {r, g, b, a};
    auto last_color = col;
    int x = x0, y = y0;
    ...
    for (auto iter = std::find_if(sv.begin(), sv.end(), find_unary); iter != sv.end(); iter = std::find_if(sv.begin(), sv.end(), find_unary))
    {
        ...
        if (sv[seg] == U'^' && seg != sv.size() - 1 && sv[seg + 1] >= U'1' && sv[seg + 1] <= U'7')
        {
            ...
            if (sv[seg + 1] == U'7')
                ...
                std::copy_n(g_color_table[sv[seg + 1] - U'0'], 4, last_color.begin());
