...
#include "game/client/menus/csmoe/BuyMenu/buypresetbutton.h"
...
void BuyPresetButton::SetText(const char *text)
{
	if (!text)
		text = "";
	const wchar_t *wtext;
	std::vector<wchar_t> unicodeText(strlen(text) + 1);
	...
	{
		vgui2::localize()->ConvertANSIToUnicode(text, unicodeText.data(), unicodeText.size() * sizeof(wchar_t));
		wtext = unicodeText.data();
	}
