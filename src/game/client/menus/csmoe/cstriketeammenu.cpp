#include "game/client/hud/hud.h"
#include "game/client/menus/csmoe/cstriketeammenu.h"

#include "ui/vgui2/interfaces/vgui/IScheme.h"
#include "ui/vgui2/interfaces/vgui/ILocalize.h"
#include "ui/vgui2/interfaces/vgui/ISurface.h"
#include "source_sdk/public/tier1/KeyValues.h"
#include "ui/vgui2/vgui_controls/ImageList.h"
#include "source_sdk/public/FileSystem.h"

#include "engine_api/types/cl_entity.h"
#include "game/shared/interfaces/cdll_dll.h"

#include "ui/vgui2/vgui_controls/TextEntry.h"
#include "ui/vgui2/vgui_controls/Button.h"
#include "ui/vgui2/vgui_controls/Panel.h"

#include "game/client/menus/CBaseViewport.h"
#include "game/shared/strings/shared_util.h"
#include "game/client/menus/csmoe/igbutton.h"

using namespace vgui2;

CCSTeamMenu::CCSTeamMenu(IViewport* pViewPort) : CTeamMenu(pViewPort)
{

}

void CCSTeamMenu::SetupControlSettings()
{
	LoadControlSettings("Resource/UI/CSO_TeamMenu.res", "GAME");
	// This text belongs to CSO's host-assigned bot variant, not master's
	// competitive or team-deathmatch rules (both allow choosing either team).
	if (auto* botRestriction = FindChildByName("BotGameLabel"))
		botRestriction->SetVisible(false);
	InvalidateLayout();
}

CCSTeamMenu::~CCSTeamMenu(void)
{
}

void CCSTeamMenu::ShowPanel(bool bShow)
{
	if (bShow)
	{
		if (gHUD.m_iIntermission || gEngfuncs.IsSpectateOnly())
			return;
	}

	BaseClass::ShowPanel(bShow);
}

void CCSTeamMenu::Update(void)
{
	BaseClass::Update();

	if (g_pViewport->GetAllowSpectators())
	{
		const auto* localPlayer = gEngfuncs.GetLocalPlayer();
		if (g_iTeamNumber == TEAM_UNASSIGNED || (localPlayer && g_PlayerExtraInfo[localPlayer->index].dead))
			SetVisibleButton("specbutton", true);
		else
			SetVisibleButton("specbutton", false);
	}
	else
		SetVisibleButton("specbutton", false);

	m_bVIPMap = false;

	char mapName[32];
	Q_FileBase(gEngfuncs.pfnGetLevelName(), mapName, sizeof(mapName));

	if (!Q_strncmp(mapName, "as_", 3))
		m_bVIPMap = true;

	if (!m_bVIPMap || (g_iTeamNumber != TEAM_CT))
		SetVisibleButton("vipbutton", false);
	else
		SetVisibleButton("vipbutton", true);

	if (g_iTeamNumber == TEAM_UNASSIGNED)
		SetVisibleButton("CancelButton", false);
	else
		SetVisibleButton("CancelButton", true);
}

void CCSTeamMenu::SetVisible(bool state)
{
	BaseClass::SetVisible(state);

	if (state)
	{
		Panel *pAutoButton = FindChildByName("autobutton");

		if (pAutoButton)
			pAutoButton->RequestFocus();
	}
}

void CCSTeamMenu::OnCommand(const char *command)
{
	if (Q_stricmp(command, "vguicancel"))
	{
		gEngfuncs.pfnClientCmd(const_cast<char *>(command));
	}

	Close();

	m_pViewPort->ShowBackGround(false);

	BaseClass::OnCommand(command);
}

void CCSTeamMenu::SetVisibleButton(const char *textEntryName, bool state)
{
	Button *entry = dynamic_cast<Button *>(FindChildByName(textEntryName));

	if (entry)
		entry->SetVisible(state);
}

void CCSTeamMenu::PaintBackground(void)
{
	BaseClass::PaintBackground();
}

void CCSTeamMenu::PerformLayout(void)
{
	BaseClass::PerformLayout();
}

void CCSTeamMenu::ApplySchemeSettings(vgui2::IScheme *pScheme)
{
	BaseClass::ApplySchemeSettings(pScheme);
}

vgui2::Panel* CCSTeamMenu::CreateControlByName(const char* controlName)
{
	if (!Q_stricmp("IGButton", controlName))
	{
		return new IGButton(this, controlName);
	}
	else
		return BaseClass::CreateControlByName(controlName);
}

void CCSTeamMenu::UpdateGameMode()
{
	SetupControlSettings();
}
