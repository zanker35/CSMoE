...
#include "engine_api/interfaces/fs_int.h"
...
bool CXashFileSystem::IsDirectory(const char *pFileName)
{
    ...
    return stat(diskPath ? diskPath : pFileName, &buf) == 0 && S_ISDIR(buf.st_mode);
}
...
{
    // FS_Write writes directly to the engine file descriptor; do not rewind it.
}
...
{
    findData_t *ptr = (findData_t *)handle;
    return ptr && ptr->search && ptr->iter > 0 && IsDirectory(ptr->search->filenames[ptr->iter - 1]);
}
