...
#include "ui/vgui2/interfaces/IHTMLChromeController.h"
#include "ui/vgui2/interfaces/vgui/ISurface.h"
#include "ui/vgui2/interfaces/IMouseControl.h"
#include "source_sdk/public/Color.h"

#include "ui/vgui2/vguimatsurface/FontTextureCache.h"

#include <vector>
#include "source_sdk/public/tier1/utldict.h"
...
public:
	// Engine HUD coordinates are screen-relative even when a VGUI panel was
	// painted immediately beforehand. Keep that drawing scope independent.
	class ScreenSpaceGuard {
	public:
		explicit ScreenSpaceGuard(BaseUISurface &surface);
		~ScreenSpaceGuard();
		ScreenSpaceGuard(const ScreenSpaceGuard &) = delete;
		ScreenSpaceGuard &operator=(const ScreenSpaceGuard &) = delete;
	private:
		BaseUISurface &m_surface;
		PaintState_t m_paintState;
		int m_textPosition[2];
		int m_textColor[4];
		vgui2::HFont m_font;
		int m_texture;
