...
#include "engine_api/interfaces/menu_int.h"
...
#include "ui/vgui2/interfaces/vgui/Cursor.h"
#include "ui/vgui2/vgui_controls/Controls.h"

#include "ui/vgui2/vgui2/vgui_surfacelib/FontManager.h"
...
extern "C" rgbdata_t *FS_LoadImage( const char *filename, const byte *buffer, size_t size );
extern "C" void FS_FreeImage( rgbdata_t *image );

extern "C" void SPR_AdjustSize( float *x, float *y, float *w, float *h );
extern "C" void SPR_AdjustSizeReverse( float *x, float *y, float *w, float *h );
extern "C" void TextAdjustSize( int *x, int *y, int *w, int *h );
extern "C" void TextAdjustSizeReverse( int *x, int *y, int *w, int *h );
...
BaseUISurface::ScreenSpaceGuard::ScreenSpaceGuard(BaseUISurface &surface) : m_surface(surface)
{
	surface.DrawFlushText();
	m_paintState = {};
	m_paintState.iTranslateX = surface._translateX;
	m_paintState.iTranslateY = surface._translateY;
	m_paintState.iScissorLeft = g_ScissorRect.left;
	m_paintState.iScissorRight = g_ScissorRect.right;
	m_paintState.iScissorTop = g_ScissorRect.top;
	m_paintState.iScissorBottom = g_ScissorRect.bottom;
	std::copy_n(surface._drawTextPos, 2, m_textPosition);
	std::copy_n(surface._drawTextColor, 4, m_textColor);
	m_font = surface.m_hCurrentFont;
	m_texture = surface.m_iCurrentTexture;
	m_scissorEnabled = g_bScissor;
	int wide, tall;
	surface.GetScreenSize(wide, tall);
	PaintState_t rootState = {};
	rootState.iScissorRight = wide;
	rootState.iScissorBottom = tall;
	surface.SetupPaintState(rootState);
	g_bScissor = true;
}

BaseUISurface::ScreenSpaceGuard::~ScreenSpaceGuard()
{
	m_surface.DrawFlushText();
	m_surface.SetupPaintState(m_paintState);
	std::copy_n(m_textPosition, 2, m_surface._drawTextPos);
	std::copy_n(m_textColor, 4, m_surface._drawTextColor);
	m_surface.m_hCurrentFont = m_font;
	m_surface.m_iCurrentTexture = m_texture;
	g_api->BindTexture(m_texture);
	g_bScissor = m_scissorEnabled;
}
...
void BaseUISurface::PopMakeCurrent(vgui2::VPANEL panel) {
	...
	if (_paintStack.size()) {
		...
		PaintState_t rootState = {};
		rootState.iScissorRight = m_iSurfaceBounds[2];
...
void BaseUISurface::DrawPrintText(const uchar32 *text, int textlen) {
    ...
    DrawSetTextPos(x + iTotalWidth - _translateX, y - _translateY);
    stackfree(pQuads);
}
...
void BaseUISurface::DrawSetTextureFile(int id, const char  * filename, int hardwareFilter, bool forceReload) {
    ...
    std::unique_ptr<rgbdata_t, decltype(&FS_FreeImage)> pic(FS_LoadImage(name, nullptr, 0), FS_FreeImage);
	if (!pic)
	{
		snprintf(name, sizeof(name), "%s.bmp", filename);

        pic.reset(FS_LoadImage(name, nullptr, 0));
		if (!pic)
		{
			DrawSetTexture(id);
			return;
...
void BaseUISurface::CalculateMouseVisible() {
    ...
    if (ui::textfuncs.pfnEnableTextInput)
        ui::textfuncs.pfnEnableTextInput(_needKB);
#endif
}
...
const void *BaseUISurface::FontDataHelper( const char *pchFontName, int &size, const char *fontFileName )
{
    ...
    if( fontFileName )
    {
        ...
        if (FileHandle_t file = vgui2::filesystem()->Open(fontFileName, "rb"))
        {
            fileSize = vgui2::filesystem()->Size(file);
            if (fileSize > 0)
            {
                buffer = malloc(fileSize);
                if (buffer && vgui2::filesystem()->Read(buffer, fileSize, file) != fileSize)
                {
                    free(buffer);
                    buffer = nullptr;
                }
        ...
        if( !buffer )
        {
            ...
            if(f)
            {
                ...
                {
                    buffer = malloc(fileSize);
                    if (buffer && fread(buffer, 1, fileSize, f) != static_cast<size_t>(fileSize))
                    {
                        free(buffer);
                        buffer = nullptr;
                    }
