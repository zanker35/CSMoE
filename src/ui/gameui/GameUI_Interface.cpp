...
#include "ui/vgui2/interfaces/IVGuiModule.h"
...
#include "ui/gameui/BasePanel.h"
#include "ui/gameui/ModInfo.h"
#include "ui/gameui/GameConsole.h"
#include "ui/gameui/LoadingDialog.h"
#include "ui/gameui/GameLoading.h"

#include "ui/gameui/GameUI_Interface.h"
...
static void OpenMenuPage(const char *command)
{
	if (!baseuifuncs || !staticPanel)
		return;
	baseuifuncs->ActivateGameUI();
	baseuifuncs->HideConsole();
	vgui2::ivgui()->PostMessage(staticPanel->GetVPanel(), new KeyValues("RunMenuCommand", "command", command), 0);
}
...
void CGameUI::Initialize(CreateInterfaceFn *factories, int count)
{
	...
	// Load every fallback first: csmoe_english also repeats the base CS tokens.
	// Loading it after cstrike_schinese would replace translated labels with English.
	const char *bundles[] = {"gameui", "valve", "vgui", "cstrike", "csmoe"};
	for (const char *language : {"english", "schinese"})
	{
		for (const char *bundle : bundles)
		{
			char filename[128];
			snprintf(filename, sizeof(filename), "resource/%s_%s.txt", bundle, language);
			vgui2::localize()->AddFile(vgui2::filesystem(), filename);
		}
	}
...
void CGameUI::StopProgressBar(bool bError, const char *failureReason, const char *extendedReason)
{
	...
	{
		// A completed map hides the GameUI parent immediately. Release the
		// loading dialog's modal restriction before that parent disappears.
		g_hLoadingDialog->SetVisible(false);
		g_hLoadingDialog->Close();
		g_hLoadingDialog = NULL;
	}
}
