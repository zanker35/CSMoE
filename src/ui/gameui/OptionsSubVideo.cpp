...
COptionsSubVideo::COptionsSubVideo(vgui2::Panel *parent) : PropertyPage(parent, NULL)
{
	...
	m_pVsync = new CCvarToggleCheckButton( this, "Vsync", "#GameUI_VSync", "gl_swapInterval" );
	...
	SetControlString("Label5", "窗口模式与分辨率在应用后立即生效。");
...
void COptionsSubVideo::ApplyVidSettings(bool bForceRefresh)
{
	...
	if ( m_pMode )
	{
		...
		int w, h;
		if (sscanf(sz, "%i x %i", &w, &h) != 2 || w <= 0 || h <= 0)
			return;
	...
	{
		int width, height;
		if (sscanf(description, "%d x %d", &width, &height) == 2 && width == p->w && height == p->h)
		{
			modeIndex = mode;
			break;
		}
	}
	// master applies modified video CVars in VID_CheckChanges each frame.
	snprintf(szCmd, sizeof(szCmd), "vid_mode %d\nfullscreen %d\n", modeIndex, !p->windowed);
