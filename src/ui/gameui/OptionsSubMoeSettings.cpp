...
COptionsSubMoeSettings::COptionsSubMoeSettings(vgui2::Panel *parent) : PropertyPage(parent, NULL)
{
	...
	m_pFloatingDamageType->SetEnabled(engine->pfnGetCvarPointer("hud_hitstyle") != nullptr);
	m_pSiFiAmmoType->SetEnabled(engine->pfnGetCvarPointer("hud_sifiammostyle") != nullptr);
	m_pAlarmStyle->SetEnabled(engine->pfnGetCvarPointer("alarm_style") != nullptr);
	m_pTexlru->SetEnabled(engine->pfnGetCvarPointer("r_texlru") != nullptr);
	m_pQZSound->SetEnabled(engine->pfnGetCvarPointer("cl_blockwdnmd") != nullptr);
	m_pAutoAim->SetEnabled(engine->pfnGetCvarPointer("in_autoaim") != nullptr);
}
